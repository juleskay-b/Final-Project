...
def draw_game_start(screen): #Julia
...
def draw_board(screen): #Ana and Julia
...
def draw_numbers(screen): #Julia
    #Draws the numbers for the base of the level - Julia
...
def select_box(screen, col, row):
    # Function to highlight the user-selected box. Box will remain highlighted and may be selected again. Julia
...
def sketch(screen, num, row, col): #Ana
...
def draw_game_over(screen, correct): #Julia
...
def handle_arrow_keys(screen, key): #Ana
    global selected_row, selected_col
...
def main_game_loop():
    ...
    while True:
        ...
        difficulty = draw_game_start(screen) #Julia
        if difficulty == "easy":
            board = generate_sudoku(9, 30)
        elif difficulty == "medium":
            board = generate_sudoku(9, 40)
